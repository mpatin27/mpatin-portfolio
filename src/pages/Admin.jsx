...
export default function Admin() {
  ...
  const [profileData, setProfileData] = useState({
    ...
    hard_skills: '', cv_file_url: '', avatar_url: '',
    phone: '', email: '', driving_license: '', languages: '', certifications: ''
  });
  const [avatarFile, setAvatarFile] = useState(null);

  // --- ÉTATS PROJETS (AJOUT short_description) ---
  const [projects, setProjects] = useState([]);
  const [projectData, setProjectData] = useState({ 
    title: '', 
    short_description: '', // <--- NOUVEAU CHAMP
    description: '', 
    tags: '', 
    demo_link: '', 
    repo_link: '' 
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('profile').update({
          ...
          hard_skills: skillsArray, languages: langsArray, certifications: certsArray,
          phone: profileData.phone, email: profileData.email, driving_license: profileData.driving_license,
          avatar_url: newAvatarUrl
        }).eq('id', 1);
      if (error) throw error; setMessage('SUCCESS: Profile updated.'); setAvatarFile(null);
  ...
  const handleProjectSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { 
        ...
        short_description: projectData.short_description, // Envoi du nouveau champ
        description: projectData.description,
        tags: tagsArray,
        demo_link: projectData.demo_link || null,
      ...
      setProjectData({ title: '', short_description: '', description: '', tags: '', demo_link: '', repo_link: '' });
  ...
  const editProject = (p) => {
    ...
    setProjectData({ 
      title: p.title, 
      short_description: p.short_description || '', // Récupération
  ...
  const handleCvSubmit = async (e) => {
    ...
    try {
      ...
      if (!editingCvId) { await supabase.from('cv_items').insert([payload]); } 
      else { await supabase.from('cv_items').update(payload).eq('id', editingCvId); }
        ...
        <div className="max-w-4xl mx-auto bg-slate-900 p-8 rounded border border-slate-700 shadow-xl">
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-6">
            <div className="border border-slate-700 p-4 rounded bg-slate-950">
               <label className="text-xs text-slate-500 uppercase mb-2 block tracking-wider">Photo Profil</label>
               <div className="flex items-center gap-4">
                 {profileData.avatar_url && <img src={profileData.avatar_url} alt="Current" className="w-16 h-16 rounded-full object-cover border-2 border-purple-500" />}
        ...
        <div className="grid lg:grid-cols-2 gap-12">
           <div className="bg-slate-900 p-8 rounded border border-slate-700 h-fit shadow-xl">
             ...
             <form onSubmit={handleProjectSubmit} className="space-y-5">
               ...
               {/* NOUVEAU CHAMP: SHORT DESC */}
               <div>
                 <label className="text-xs text-green-400 uppercase mb-1 block tracking-wider font-bold">Short Description (Card view)</label>
                 <textarea value={projectData.short_description} onChange={e => setProjectData({...projectData, short_description: e.target.value})} className="w-full bg-slate-950 border border-green-900/50 p-3 rounded text-white focus:border-green-500 outline-none" rows="2" placeholder="Accroche courte pour la grille..." />
               </div>

               <div>
                 <label className="text-xs text-slate-500 uppercase mb-1 block tracking-wider">Full Description (Technical/Modal)</label>
                 <textarea value={projectData.description} onChange={e => setProjectData({...projectData, description: e.target.value})} className="w-full bg-slate-950 border border-slate-700 p-3 rounded text-white focus:border-green-500 outline-none" rows="6" required placeholder="Détails techniques complets..." />
               </div>
